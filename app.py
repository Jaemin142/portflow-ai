...
">
    ...
    <div style="font-size:0.9rem; color:#DBEAFE; line-height:1.6;">
       관세청 수출입 데이터로 국가·품목별 수요 변화를 예측하고,
       우선 점검할 공급망 리스크 조합을 선별합니다.
    </div>
</div>
""", unsafe_allow_html=True)
...
with tab2:
    ...
    st.markdown(
        ...
        중량·금액·무역수지 변화를 통해 수요 증가, 감소, 변동성 확대 여부를 확인합니다.
        """
    )
...
with tab3:
    ...
    st.markdown(
        f"""
        **{selected_country} - {selected_hs} - {metric_option}** 기준으로 향후 3개월 수요를 예측합니다.

        - 모델: **{forecast_model_type}**
        - 지표: **{metric_option}**
        - 단위: **{target_unit}**
        """
    )

    with st.expander("예측 모델 설명 보기", expanded=False):
        st.markdown(
            """
            월별 수출입 실적을 시간 인덱스로 변환한 뒤,
            `LinearRegression` 모델로 향후 3개월 값을 예측합니다.

            - 입력 데이터: 선택 국가·품목군의 월별 수출입 지표
            - 예측 방식: 시간 순서 기반 추세 예측
            - 예외 처리: 데이터가 6개월 미만이면 최근 3개월 평균값 사용
            """
        )
...
with tab5:
    ...
    st.markdown(
        f"""
        선택한 **{selected_country} - {selected_hs}** 조합의 수요 변화, 변동성, 불균형, 예측 변화율을 종합해 리스크 점수를 산정합니다.

        이 점수는 실제 지연 확률이 아니라, 우선 모니터링 대상을 선별하기 위한 조기경보 지표입니다.
        """
    )
...
with tab7:
    ...
    st.markdown(
        f"""
        전체 국가·품목 조합을 대상으로 **{metric_option}** 기준 리스크 점수를 계산합니다.

수요 변동성뿐 아니라 국가별 수입의존도와 총교역비중을 반영해 우선 점검 대상을 선별합니다.
        """
    )
